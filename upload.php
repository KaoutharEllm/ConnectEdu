...
// Email content setup
$mail->Subject = $uploadSuccess ? 'New Materials Uploaded' : 'New Announcement Posted';
$mailContent = $uploadSuccess ?
    "New educational materials have been uploaded to <strong>{$course['titre']}</strong> by <strong>{$course['nom']} {$course['prenom']}</strong>. Please check them out now!" :
    "A new announcement has been made in <strong>{$course['titre']}</strong> by <strong>{$course['nom']} {$course['prenom']}</strong>. Please check it out now!";
$mail->Body = <<<HTML
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <div class="container">
        ...
        <div class="content">
            <p>{$mailContent}</p>
            <a href="http://localhost/connectEdu/logging.html" class="button">View Details</a>
        </div>
    </div>
</body>
</html>
